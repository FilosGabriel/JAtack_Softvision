...
public class ImageLoader {
    ...
    private ImageLoader() throws Exception {
        FileInputStream input = new FileInputStream(imagesBasePath + "alien.png");
        Image image = new Image(input);
        input.close();

        images.put(ImageType.ALIEN, image);
        input=new FileInputStream(imagesBasePath+"heart.png");
        image=new Image(input);
        input.close();
        images.put(ImageType.HEART,image);

                input = new FileInputStream(imagesBasePath + "plane.png");
        image = new Image(input);
        input.close();
